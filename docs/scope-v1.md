...
| VPS | Scaleway. Commandés automatiquement par l'outil quand la capacité est atteinte |
| Versionnement | Un repo GitHub privé par site, branches `staging` / `production` |
...
- Serveur choisi automatiquement : carte d'information « Serveur : vps-02 (12/25 sites) » ou « Un nouveau serveur sera commandé (≈ X €/mois) ».
...
- **Serveurs** : liste, capacité, santé (SSH, Caddy, espace disque), bouton « Commander un serveur », limite de sites par serveur (défaut 25), retrait d'un serveur vide.
- **Agence** : domaine de preview, contact Gandi par défaut (obligatoire pour l'achat), offre Scaleway par défaut.
